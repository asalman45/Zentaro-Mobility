...
const staticModels: Record<string, ModelData> = {
  "zentaro-thunder": {
    ...
    variants: [
      { id: 1, name: "Thunder LFP Standard", batteryType: "LFP", price: 549000, rangeKm: 120, topSpeedKmh: 95 },
      { id: 2, name: "Thunder LFP Pro (Dual Battery)", batteryType: "LFP", price: 679000, rangeKm: 220, topSpeedKmh: 105 },
    ],
  },
  "zentaro-alpha": {
    slug: "zentaro-alpha",
    ...
    variants: [
      { id: 3, name: "Alpha Graphene Commuter", batteryType: "Graphene", price: 369000, rangeKm: 90, topSpeedKmh: 75 },
      { id: 4, name: "Alpha LFP Long-Range", batteryType: "LFP", price: 429000, rangeKm: 140, topSpeedKmh: 85 },
    ],
  },
  "zentaro-breeze": {
    slug: "zentaro-breeze",
    ...
    variants: [
      { id: 5, name: "Breeze Graphene Standard", batteryType: "Graphene", price: 279000, rangeKm: 70, topSpeedKmh: 60 },
...
export default function ReserveCheckoutPage({ params }: PageProps) {
  ...
  // Unwrap params using React.use() equivalent or simple promise useEffect
  useEffect(() => {
    params.then((p) => {
      setResolvedSlug(p.slug);
      const bike = staticModels[p.slug];
      if (bike) {
        setModel(bike);
        
        // Match variant from search parameters or fallback to default
        const vId = Number(searchParams.get("variant"));
        const matchV = bike.variants.find((v) => v.id === vId) || bike.variants[0];
        setSelectedVariant(matchV);

        // Match color
        const matchC = searchParams.get("color") || bike.colors[0].name;
...
// ChevronLeft helper in case of missing imports
